...
pub struct DisconnectingCircuitBreaker {
    #[serde(flatten)]
    pub base: super::Breaker,
}
impl crate::base::CimElement for DisconnectingCircuitBreaker {
    fn mrid(&self) -> &str { &self.base.base.base.base.base.base.base.id }
    fn type_name(&self) -> &'static str { "DisconnectingCircuitBreaker" }
    ...
    fn to_block(&self) -> crate::base::RdfBlock {
        ...
        block.type_name = "DisconnectingCircuitBreaker".to_string();
        block
    }
}

impl DisconnectingCircuitBreaker {
    pub fn from_block(b: &crate::base::RdfBlock) -> Self {
        ...
        for (key, val) in &b.fields {
            match key.as_str() {
                "Switch.locked" => {
                    match val {
                        crate::base::FieldValue::Text(sv) => { obj.base.base.base.locked = Some(sv.trim() == "true"); }
                        crate::base::FieldValue::TextList(svs) => {
                            if let Some(sv) = svs.last() { obj.base.base.base.locked = Some(sv.trim() == "true"); }
                        }
                        _ => {}
                    }
                }
                "Switch.normalOpen" => {
                    match val {
                        crate::base::FieldValue::Text(sv) => { obj.base.base.base.normal_open = Some(sv.trim() == "true"); }
                        crate::base::FieldValue::TextList(svs) => {
                            if let Some(sv) = svs.last() { obj.base.base.base.normal_open = Some(sv.trim() == "true"); }
                        }
                        _ => {}
                    }
                }
                "Switch.open" => {
                    match val {
                        crate::base::FieldValue::Text(sv) => { obj.base.base.base.open = Some(sv.trim() == "true"); }
                        crate::base::FieldValue::TextList(svs) => {
                            if let Some(sv) = svs.last() { obj.base.base.base.open = Some(sv.trim() == "true"); }
                        }
                        _ => {}
                    }
                }
                "Switch.ratedCurrent" => {
                    match val {
                        crate::base::FieldValue::Text(sv) => { if let Ok(v) = sv.trim().parse() { obj.base.base.base.rated_current = Some(v); } }
                        crate::base::FieldValue::TextList(svs) => {
                            if let Some(sv) = svs.last() { if let Ok(v) = sv.trim().parse() { obj.base.base.base.rated_current = Some(v); } }
                        }
                        _ => {}
                    }
                }
                "Switch.retained" => {
                    match val {
                        crate::base::FieldValue::Text(sv) => { obj.base.base.base.retained = Some(sv.trim() == "true"); }
                        crate::base::FieldValue::TextList(svs) => {
                            if let Some(sv) = svs.last() { obj.base.base.base.retained = Some(sv.trim() == "true"); }
                        }
                        _ => {}
                    }
                }
