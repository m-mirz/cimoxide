...
impl crate::base::CimElement for BusbarSection {
    fn mrid(&self) -> &str { &self.base.base.base.base.base.id }
    fn type_name(&self) -> &'static str { "BusbarSection" }
    ...
    fn to_block(&self) -> crate::base::RdfBlock {
        ...
        block.type_name = "BusbarSection".to_string();
        if let Some(v) = self.ip_max {
            block.fields.insert("BusbarSection.ipMax".into(), crate::base::FieldValue::Text(v.to_string()));
        }
        block
    }
}
    ...
    pub fn from_block(b: &crate::base::RdfBlock) -> Self {
        ...
        for (key, val) in &b.fields {
            match key.as_str() {
                "BusbarSection.ipMax" => {
                    match val {
                        crate::base::FieldValue::Text(sv) => { if let Ok(v) = sv.trim().parse() { obj.ip_max = Some(v); } }
                        crate::base::FieldValue::TextList(svs) => {
                            if let Some(sv) = svs.last() { if let Ok(v) = sv.trim().parse() { obj.ip_max = Some(v); } }
                        }
                        _ => {}
                    }
                }
