...
impl crate::base::CimElement for AnalogLimit {
    fn mrid(&self) -> &str { &self.base.base.id }
    fn type_name(&self) -> &'static str { "AnalogLimit" }
    ...
    fn to_block(&self) -> crate::base::RdfBlock {
        ...
        block.type_name = "AnalogLimit".to_string();
        if let Some(ref v) = self.limit_set {
            block.fields.insert("AnalogLimit.LimitSet".into(), crate::base::FieldValue::Resource(v.mrid.clone()));
        }
        if let Some(v) = self.value {
            block.fields.insert("AnalogLimit.value".into(), crate::base::FieldValue::Text(v.to_string()));
        }
        block
    }
}
