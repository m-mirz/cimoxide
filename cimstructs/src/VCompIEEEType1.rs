...
pub struct VCompIEEEType1 {
    #[serde(flatten)]
    pub base: super::VoltageCompensatorDynamics,
...
impl crate::base::CimElement for VCompIEEEType1 {
    fn mrid(&self) -> &str { &self.base.base.base.id }
    fn type_name(&self) -> &'static str { "VCompIEEEType1" }
    ...
    fn to_block(&self) -> crate::base::RdfBlock {
        ...
        block.type_name = "VCompIEEEType1".to_string();
        if let Some(v) = self.rc {
            block.fields.insert("VCompIEEEType1.rc".into(), crate::base::FieldValue::Text(v.to_string()));
        }
        if let Some(v) = self.tr {
            block.fields.insert("VCompIEEEType1.tr".into(), crate::base::FieldValue::Text(v.to_string()));
        }
        if let Some(v) = self.xc {
            block.fields.insert("VCompIEEEType1.xc".into(), crate::base::FieldValue::Text(v.to_string()));
        }
        block
    }
}
    ...
    pub fn from_block(b: &crate::base::RdfBlock) -> Self {
        ...
        for (key, val) in &b.fields {
            match key.as_str() {
                "VCompIEEEType1.rc" => {
                    match val {
                        crate::base::FieldValue::Text(sv) => { if let Ok(v) = sv.trim().parse() { obj.rc = Some(v); } }
                        crate::base::FieldValue::TextList(svs) => {
                            if let Some(sv) = svs.last() { if let Ok(v) = sv.trim().parse() { obj.rc = Some(v); } }
                        }
                        _ => {}
                    }
                }
