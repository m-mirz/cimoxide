...
pub struct AnalogLimitSet {
    #[serde(flatten)]
    pub base: super::LimitSet,
    /// The Measurements using the LimitSet.
    pub measurements: Vec<super::base::MridRef>,
}
impl crate::base::CimElement for AnalogLimitSet {
    fn mrid(&self) -> &str { &self.base.base.id }
    fn type_name(&self) -> &'static str { "AnalogLimitSet" }
    ...
    fn to_block(&self) -> crate::base::RdfBlock {
        ...
        block.type_name = "AnalogLimitSet".to_string();
        if !self.measurements.is_empty() {
            block.fields.insert("AnalogLimitSet.Measurements".into(), crate::base::FieldValue::ResourceList(self.measurements.iter().map(|r| r.mrid.clone()).collect()));
        }
        block
    }
}
        ...
        for (key, val) in &b.fields {
            match key.as_str() {
                "AnalogLimitSet.Measurements" => {
                    match val {
                        crate::base::FieldValue::Resource(sv) => obj.measurements.push(crate::base::MridRef { mrid: sv.clone() }),
                        crate::base::FieldValue::ResourceList(svs) => {
                            for sv in svs { obj.measurements.push(crate::base::MridRef { mrid: sv.clone() }); }
                        }
                        _ => {}
                    }
                }
                "LimitSet.isPercentageLimits" => {
                    match val {
                        crate::base::FieldValue::Text(sv) => { obj.base.is_percentage_limits = Some(sv.trim() == "true"); }
                        crate::base::FieldValue::TextList(svs) => {
                            if let Some(sv) = svs.last() { obj.base.is_percentage_limits = Some(sv.trim() == "true"); }
                        }
                        _ => {}
                    }
                }
