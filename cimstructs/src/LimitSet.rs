...
pub struct LimitSet {
    ...
    pub is_percentage_limits: Option<bool>,
}
impl crate::base::CimElement for LimitSet {
    fn mrid(&self) -> &str { &self.base.id }
    fn type_name(&self) -> &'static str { "LimitSet" }
    ...
    fn to_block(&self) -> crate::base::RdfBlock {
        ...
        block.type_name = "LimitSet".to_string();
        if let Some(v) = self.is_percentage_limits {
            block.fields.insert("LimitSet.isPercentageLimits".into(), crate::base::FieldValue::Text(v.to_string()));
        }
        block
    }
}
        ...
        for (key, val) in &b.fields {
            match key.as_str() {
                "LimitSet.isPercentageLimits" => {
                    match val {
                        crate::base::FieldValue::Text(sv) => { obj.is_percentage_limits = Some(sv.trim() == "true"); }
                        crate::base::FieldValue::TextList(svs) => {
                            if let Some(sv) = svs.last() { obj.is_percentage_limits = Some(sv.trim() == "true"); }
                        }
                        _ => {}
                    }
                }
