...
pub struct PhaseTapChangerNonLinear {
    #[serde(flatten)]
    ...
    pub voltage_step_increment: Option<f64>,
...
impl crate::base::CimElement for PhaseTapChangerNonLinear {
    fn mrid(&self) -> &str { &self.base.base.base.base.id }
    fn type_name(&self) -> &'static str { "PhaseTapChangerNonLinear" }
    ...
    fn to_block(&self) -> crate::base::RdfBlock {
        ...
        block.type_name = "PhaseTapChangerNonLinear".to_string();
        if let Some(v) = self.voltage_step_increment {
            block.fields.insert("PhaseTapChangerNonLinear.voltageStepIncrement".into(), crate::base::FieldValue::Text(v.to_string()));
        }
        if let Some(v) = self.x_max {
            block.fields.insert("PhaseTapChangerNonLinear.xMax".into(), crate::base::FieldValue::Text(v.to_string()));
        }
        if let Some(v) = self.x_min {
            block.fields.insert("PhaseTapChangerNonLinear.xMin".into(), crate::base::FieldValue::Text(v.to_string()));
        }
        block
    }
}

impl PhaseTapChangerNonLinear {
        ...
        for (key, val) in &b.fields {
            match key.as_str() {
                "PhaseTapChangerNonLinear.voltageStepIncrement" => {
                    match val {
                        crate::base::FieldValue::Text(sv) => { if let Ok(v) = sv.trim().parse() { obj.voltage_step_increment = Some(v); } }
                        crate::base::FieldValue::TextList(svs) => {
                            if let Some(sv) = svs.last() { if let Ok(v) = sv.trim().parse() { obj.voltage_step_increment = Some(v); } }
                        }
                        _ => {}
                    }
                }
