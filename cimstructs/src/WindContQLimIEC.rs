...
impl crate::base::CimElement for WindContQLimIEC {
    fn mrid(&self) -> &str { &self.base.id }
    fn type_name(&self) -> &'static str { "WindContQLimIEC" }
    ...
    fn to_block(&self) -> crate::base::RdfBlock {
        ...
        block.type_name = "WindContQLimIEC".to_string();
        if let Some(v) = self.qmax {
            block.fields.insert("WindContQLimIEC.qmax".into(), crate::base::FieldValue::Text(v.to_string()));
        }
        if let Some(v) = self.qmin {
            block.fields.insert("WindContQLimIEC.qmin".into(), crate::base::FieldValue::Text(v.to_string()));
        }
        block
    }
}

impl WindContQLimIEC {
        ...
        for (key, val) in &b.fields {
            match key.as_str() {
                "WindContQLimIEC.qmax" => {
                    match val {
                        crate::base::FieldValue::Text(sv) => { if let Ok(v) = sv.trim().parse() { obj.qmax = Some(v); } }
                        crate::base::FieldValue::TextList(svs) => {
                            if let Some(sv) = svs.last() { if let Ok(v) = sv.trim().parse() { obj.qmax = Some(v); } }
                        }
                        _ => {}
                    }
                }
                "WindContQLimIEC.qmin" => {
                    match val {
                        crate::base::FieldValue::Text(sv) => { if let Ok(v) = sv.trim().parse() { obj.qmin = Some(v); } }
                        crate::base::FieldValue::TextList(svs) => {
                            if let Some(sv) = svs.last() { if let Ok(v) = sv.trim().parse() { obj.qmin = Some(v); } }
                        }
                        _ => {}
                    }
                }
