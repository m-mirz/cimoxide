...
pub struct PFVArType2IEEEVArController {
    #[serde(flatten)]
...
impl crate::base::CimElement for PFVArType2IEEEVArController {
    fn mrid(&self) -> &str { &self.base.base.base.id }
    fn type_name(&self) -> &'static str { "PFVArType2IEEEVArController" }
    ...
    fn to_block(&self) -> crate::base::RdfBlock {
        ...
        block.type_name = "PFVArType2IEEEVArController".to_string();
        if let Some(v) = self.exlon {
            block.fields.insert("PFVArType2IEEEVArController.exlon".into(), crate::base::FieldValue::Text(v.to_string()));
        }
        if let Some(v) = self.ki {
            block.fields.insert("PFVArType2IEEEVArController.ki".into(), crate::base::FieldValue::Text(v.to_string()));
        }
        if let Some(v) = self.kp {
            block.fields.insert("PFVArType2IEEEVArController.kp".into(), crate::base::FieldValue::Text(v.to_string()));
        }
        if let Some(v) = self.qref {
            block.fields.insert("PFVArType2IEEEVArController.qref".into(), crate::base::FieldValue::Text(v.to_string()));
        }
        if let Some(v) = self.vclmt {
            block.fields.insert("PFVArType2IEEEVArController.vclmt".into(), crate::base::FieldValue::Text(v.to_string()));
        }
        if let Some(v) = self.vref {
            block.fields.insert("PFVArType2IEEEVArController.vref".into(), crate::base::FieldValue::Text(v.to_string()));
        }
        if let Some(v) = self.vs {
            block.fields.insert("PFVArType2IEEEVArController.vs".into(), crate::base::FieldValue::Text(v.to_string()));
        }
        block
    }
}

impl PFVArType2IEEEVArController {
        ...
        for (key, val) in &b.fields {
            match key.as_str() {
                ...
                "PFVArType2IEEEVArController.qref" => {
                    match val {
                        crate::base::FieldValue::Text(sv) => { if let Ok(v) = sv.trim().parse() { obj.qref = Some(v); } }
                        crate::base::FieldValue::TextList(svs) => {
                            if let Some(sv) = svs.last() { if let Ok(v) = sv.trim().parse() { obj.qref = Some(v); } }
                        }
                        _ => {}
                    }
                }
