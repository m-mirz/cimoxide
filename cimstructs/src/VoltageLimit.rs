...
impl crate::base::CimElement for VoltageLimit {
    fn mrid(&self) -> &str { &self.base.base.id }
    fn type_name(&self) -> &'static str { "VoltageLimit" }
    ...
    fn to_block(&self) -> crate::base::RdfBlock {
        ...
        block.type_name = "VoltageLimit".to_string();
        if let Some(v) = self.normal_value {
            block.fields.insert("VoltageLimit.normalValue".into(), crate::base::FieldValue::Text(v.to_string()));
        }
        if let Some(v) = self.value {
            block.fields.insert("VoltageLimit.value".into(), crate::base::FieldValue::Text(v.to_string()));
        }
        block
    }
}
