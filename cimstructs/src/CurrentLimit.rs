...
impl crate::base::CimElement for CurrentLimit {
    fn mrid(&self) -> &str { &self.base.base.id }
    fn type_name(&self) -> &'static str { "CurrentLimit" }
    ...
    fn to_block(&self) -> crate::base::RdfBlock {
        ...
        block.type_name = "CurrentLimit".to_string();
        if let Some(v) = self.normal_value {
            block.fields.insert("CurrentLimit.normalValue".into(), crate::base::FieldValue::Text(v.to_string()));
        }
        if let Some(v) = self.value {
            block.fields.insert("CurrentLimit.value".into(), crate::base::FieldValue::Text(v.to_string()));
        }
        block
    }
}
