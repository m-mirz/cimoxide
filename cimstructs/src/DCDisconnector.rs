...
pub struct DCDisconnector {
    #[serde(flatten)]
    pub base: super::DCSwitch,
}
impl crate::base::CimElement for DCDisconnector {
    fn mrid(&self) -> &str { &self.base.base.base.base.base.id }
    fn type_name(&self) -> &'static str { "DCDisconnector" }
    ...
    fn to_block(&self) -> crate::base::RdfBlock {
        ...
        block.type_name = "DCDisconnector".to_string();
        block
    }
}
        ...
        for (key, val) in &b.fields {
            match key.as_str() {
                "DCConductingEquipment.ratedUdc" => {
                    match val {
                        crate::base::FieldValue::Text(sv) => { if let Ok(v) = sv.trim().parse() { obj.base.base.rated_udc = Some(v); } }
                        crate::base::FieldValue::TextList(svs) => {
                            if let Some(sv) = svs.last() { if let Ok(v) = sv.trim().parse() { obj.base.base.rated_udc = Some(v); } }
                        }
                        _ => {}
                    }
                }
