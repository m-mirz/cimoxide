...
and counts as reported by `cargo run -p cimoxide-gen -- --skip-report`. Its total (6843) matches
...
`cargo run -p cimoxide-gen -- --rule-report` also breaks the generated-vs-skipped
