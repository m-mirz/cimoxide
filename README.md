...
function in `cimvalidation/`. The 186 SPARQL constraints (all currently implemented, see
...
| 181 | SPARQL-derived constraints | `sh:sparql` constraints and `sh:target SPARQLTarget` targets both require evaluating an arbitrary SPARQL query at runtime; `cimgen` does not retain the query text, so both have a hand-written implementation instead (see "SPARQL Check Coverage" below). `cimsparql` now provides an evaluator (see ["SPARQL"](#sparql) above), but validation is deliberately not wired to it — these checks stay hand-written. Note: this 181 is *not* the same count as the 186 in "SPARQL Check Coverage" — this one is every distinct `(property, component, sh:name)` skip entry, deduped per TTL file and not split on `sh:name`'s `|`-joined compound values, so it undercounts relative to the coverage table's per-rule-name count. Cross-tool: matches cimgo's "SPARQL-derived constraints" row (181) exactly — see ["Comparing skip categories with cimgo"](#comparing-skip-categories-with-cimgo) below. |
...
| 4 | `sh:targetSubjectsOf` / `sh:targetObjectsOf` | Property-based target mechanism; the shape is recognised but there's no concrete class for `cimgen` to generate a per-class check against. All 4 are the `IdentifiedObject.*:stringLength` rules, which upstream rewrote from `sh:sparql` into plain `sh:maxLength` / `sh:length` — the constraint components are now imported, but the property-based target still blocks codegen, so all 4 keep their hand-written coverage in `cimvalidation/src/sparql/common.rs` under those exact `sh:name`s. (`C:600:ALL:NA:FBOD4` was the fifth entry here before the bump; upstream reshaped it into a `sh:SPARQLTarget`, so it now lands in the SPARQL row above and stays implemented in `common_solved_mas.rs`.) Nothing here is actually unchecked — the "SPARQL Check Coverage" table below still reports 100%. Cross-tool: cimgo reports 3 rather than 4, because its `shaclimport` does not recognise `sh:length` and drops the `energyIdentCodeEic` shape before it reaches the skip report. |
...
| 1 | `sh:maxCount 0` on a non-list field | `C:301:EQ:PowerTransformer:associationNotUsed`, which upstream rewrote from `sh:sparql` into `sh:maxCount 0` on `cim:ConductingEquipment.BaseVoltage`. `cimgen` only supports `sh:maxCount 1`; a `0` upper bound is a presence check on a scalar field, which the generator has no template for. Implemented by hand in `cimvalidation/src/sparql/equipment.rs`. Cross-tool: cimgo generates this one instead and removed its hand-written copy — the single row with no cimgo counterpart. |
| 1 | `sh:minLength<=1` unreachable | `C:600:ALL:Model.modelingAuthoritySet:marp10-12`, which upstream rewrote from `sh:sparql` into `sh:minLength 1`. An absent string and a present-but-empty one both decode to `""`, so the generated empty-value guard would swallow every value the check could reject — emitting it would claim coverage for a function that can never fire. Implemented by hand in `cimvalidation/src/sparql/common.rs`, where the empty case is reachable. Cross-tool: matches cimgo's identically-named row (1) exactly. |
...
cimoxide's and cimgo's skip-category tables cover the same 74 TTL files. Both track the same
`application-profiles-library` commit (`d8b2d21`), but their non-SPARQL constraint totals
differ by one — 12,270 here vs. 12,269 in cimgo — because cimgo's `shaclimport` does not
recognise `sh:length`, so the `IdentifiedObject.energyIdentCodeEic` shape upstream rewrote at
`d8b2d21` never enters its constraint list. The rule is still checked by hand on both sides.
Rows describing the identical underlying SHACL feature are given matching wording and
cross-referenced inline above.
...
- The SPARQL-derived-constraints row (181) matches cimgo's identically-named row exactly.
  `sh:targetSubjectsOf`/`sh:targetObjectsOf` is 4 here vs. 3 in cimgo — the missing
  `energyIdentCodeEic` shape described above.
- `sh:minLength<=1` unreachable (1) matches cimgo's identically-named row: both tools refuse
  to emit a check that their empty-string guard would make unreachable.
- cimgo has rows with no cimoxide equivalent — `sh:required` on `float`/`bool` fields and
  ...
  both of which let it generate real checks instead of skipping them.
- One row is cimoxide-only: `sh:maxCount 0` on a non-list field. cimgo generates that check
  (`eq:PowerTransformer-associationNotUsed`) and dropped its hand-written copy at the bump;
  `cimgen` has no template for a zero upper bound and keeps `equipment.rs`'s hand-written
  check instead. Same rule, same behaviour, different side of the generated/hand-written line.
